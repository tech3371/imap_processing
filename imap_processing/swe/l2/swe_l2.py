...
def calculate_phase_space_density(l1b_dataset: xr.Dataset) -> None:
    ...
    Calculate phase space density, fv, in units of s^3/cm^6
        ...
        tau = sampling time, in s. It's the ACQ_DURATION from packet. Use first
            quarter cycle's value per SWE team.
    ...
    electron_speed = {
        k: np.sqrt(2 * energy / ELECTRON_MASS) for k, energy in particle_energy.items()
    }
    ...
    for esa_table_num in electron_speed:
        ...
        indices = np.where(l1b_dataset["esa_table_num"].values[:, 0] == esa_table_num)[
            0
        ]
