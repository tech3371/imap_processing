...
class ViewTabInfo:
    ...
    def __init__(
        ...
        self.sensor = sensor
        self.three_d_collapsed = three_d_collapsed
        self.collapse_table = collapse_table
...
def get_collapse_pattern_shape(
    ...
    return (unique_columns.shape[1], unique_rows.shape[0])
...
def calculate_acq_time_per_step(low_stepping_tab: dict) -> np.ndarray:
    ...
    dwell_faction = tunable_values["dwell_fraction_percentage"]
    ...
    total_non_acq_time = float(sector_time) * (100.0 - dwell_faction) / 100.0
    ...
    non_adjusted_hv_settle_per_step = (
        total_non_acq_time - sector_margin_ms
    ) / num_steps_data
