...
def convert_to_rates(dataset: xr.Dataset, descriptor: str) -> np.ndarray:
    ...
    # Now solve denominator based on data product type
    if descriptor in [
        ...
        # Clean up attributes added by cdflib that are not needed
        for variable in ["k_factor", "voltage_table", "acquisition_time_per_step"]:
            # remove new DEPEND_1 added by cdflib
            # TODO: revisit this after discussion with Bryan Harter
            if "DEPEND_1" in dataset[variable].attrs:
                del dataset[variable].attrs["DEPEND_1"]
    elif descriptor in [
