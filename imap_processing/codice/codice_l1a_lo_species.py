...
def l1a_lo_species(unpacked_dataset: xr.Dataset, lut_file: Path) -> xr.Dataset:
    ...
    # lookup in LUT table.
    table_id = np.unique(unpacked_dataset["table_id"].values)[0]
    view_id = np.unique(unpacked_dataset["view_id"].values)[0]
    apid = np.unique(unpacked_dataset["pkt_apid"].values)[0]
    plan_id = np.unique(unpacked_dataset["plan_id"].values)[0]
    plan_step = np.unique(unpacked_dataset["plan_step"].values)[0]
    ...
    if view_tab_obj.apid == CODICEAPID.COD_LO_SW_SPECIES_COUNTS:
        ...
        logger.error("Unknown apid in Lo species processing.")
    ...
    l1a_dataset["k_factor"] = xr.DataArray(
        ...
        dims=("spin_sector",),
        attrs=cdf_attrs.get_variable_attributes("k_factor"),
    )
    ...
    for idx, species in enumerate(species_names):
        ...
        species_attrs["CATDESC"] = (
            species_attrs["CATDESC"]
            .replace("{species}", species)
            .replace("{direction}", direction)
        )
        species_attrs["FIELDNAM"] = (
            species_attrs["FIELDNAM"]
            .replace("{species}", species)
            .replace("{direction}", direction)
        )
